...
| `anthropic_status.py` | Reads Claude's **public service-status page** (`status.claude.com/api/v2/summary.json`) so the panel can flag outages; caches to `~/.usage/anthropic_status_cache.json` (TTL 5 min, 60s backoff after a failure), mirroring `pricing.py`'s fetch/cache shape. Inspects **only** the `Claude Code` and `Claude API (api.anthropic.com)` components — never the page's overall `indicator`, which also reflects unrelated incidents (an Office add-in outage) and would false-alarm. A status page is **not a usage API**; this does not violate the no-LLM-usage-API invariant. |
