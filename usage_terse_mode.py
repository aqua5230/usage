...
_DEFAULT_INSTRUCTION: dict[str, str] = {
    "zh-TW": (
        ...
        "在後面補十個字以內的白話解釋，之後直接用。工具或子代理的輸出不要原文轉貼：先讀懂再用白話"
        "重寫成結論，只有程式碼、指令、路徑、錯誤訊息照原文保留。不要比喻、不要為了親切多寫。名詞化"
        "還原成動詞"
    ...
    "en": (
        ...
        " use in eight words or fewer, then just use it. Never paste a tool's or subagent's out"
        "put verbatim: read it, then rewrite it as a plain-language conclusion — only code, co"
        "mmands, paths, and error messages stay verbatim. No analogies, no warmth padding. Zombi"
    ...
    "zh-CN": (
        ...
        "在后面补十个字以内的白话解释，之后直接用。工具或子代理的输出不要原文转贴：先读懂再用白话"
        "重写成结论，只有代码、指令、路径、错误信息照原文保留。不要比喻、不要为了亲切多写。名词化"
        "还原成动词"
    ...
    "ja": (
        ...
        "内で平易な説明を補足し、以後はそのまま使用してください。ツールやサブエージェントの出力を"
        "そのまま貼り付けないこと。まず理解し、平易な言葉で結論として書き直す。原文のまま残すのは"
        "コード、コマンド、パス、エラーメッセージだけ。比喩や、親しみやすさを出すため"
    ...
    "ko": (
        ...
        " 도구나 서브에이전트의 출력을 그대로 붙여넣지 말 것: 먼저 "
        "이해한 뒤 쉬운 말로 결론으로 다시 쓰고, 코드·명령어·경로·오류 메시지만 "
        "원문 그대로 둔다. 비유나 친근감을 주기 위한 군더더기는 "
        "빼주세요. 명사화는 동사로 (수정을 진행한다 → 고"
