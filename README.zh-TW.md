...
- **請 Claude（還有 Codex）講話精簡一點，省輸出 token。** 選單列開關「精簡模式」，讓 Claude Code 整個對話回答更簡短，有裝 Codex CLI 的話也會一起套用——內容不變、字變少，但程式碼、指令、檔案路徑、錯誤訊息一個字都不會被縮減。完全本機、預設關閉。
  ```
  開啟前：「好的，我來看一下！這個問題滿常見的，通常是你指的路徑寫錯了，
           或者檔案根本不在你以為的那個資料夾裡，我先確認一下目前的
           目錄結構，再幫你抓出真正的原因。」
  開啟後：「路徑錯了，檔案不在那個資料夾。」
  ```
  跟任何風格指令一樣，這是「請求」不是硬規定——對話拉得太長還是可能慢慢淡忘、飄回 Claude 原本的講話方式，提醒它一次就會恢復。
...
- **把一整套 AI 團隊帶進 Claude Code。** AI 人才市場面板能依情境把整套角色人設檔——律師事務所、軟體工作室等——一次安裝進 `~/.claude/agents/`。挑一個角色安裝好，下次在 Claude Code 對話裡點名就能用。靠內建的本機 CLI 運作，免帳號、不連網。
