...
`usage` 是一個 macOS menu bar（螢幕右上角的選單列）小工具，把 **Claude Code、Codex 跟 Gemini** 的用量釘在你的螢幕右上角。點開可以看到 Session、Weekly、各專案用量（今日 / 7 日 / 月），以及今日 token 用量與成本估算。

不呼叫 Anthropic / OpenAI / Google 的 API（接口）、也不讀 Keychain（macOS 內建的密碼保險箱），所以不會發生「自己每分鐘 ping 一次也算用量」這種事。
...
### Gemini (Antigravity) 用量

usage 會掃描 Antigravity CLI 代理在本機留下的對話紀錄檔（`~/.gemini/antigravity-cli/brain/*/transcript.jsonl`）。它會讀取這些紀錄中每個對話輪次的輸入與輸出 token 數量，計算出 5 小時與 7 天的用量百分比與成本。

若未安裝 Antigravity 或該資料夾不存在，Gemini 卡片會自動隱藏，不影響其他卡片正常運作。
...
## 多裝置同步 (Multi-device synchronization)

如果在多台機器（例如本機 Mac 和遠端開發伺服器）上使用 Claude Code，`usage` 可以透過 SSH 自動同步用量狀態與專案資料夾：

1. 在本機的 `~/.claude/settings.json` 中設定遠端主機：
   ```json
   {
     "usage": {
       "remotes": ["my-server", "work-vm"]
     }
   }
   ```
2. 確保本機可以透過 SSH 金鑰免密碼登入這些遠端主機。
3. `usage` 每隔 5 分鐘會在背景自動執行同步，使用 `ssh`、`scp` 和 `rsync` 來雙向同步專案資料夾、將 statusline hook 腳本複製到遠端，並把遠端的用量狀態檔拉回本機。
