...
- **Windows 面板控制選單現在能完整顯示，且分組與 macOS 一致**：重設面板位置已移至 Windows 專屬的底部操作；「更換面板」與「隱藏區塊」會在可捲動選單內就地展開，不再開啟可能被裁切的側邊子選單。
- **拖曳 Windows 面板時不再誤排額度卡片**：Windows shim 會停用共用的卡片排序手勢，但仍保留已儲存的卡片順序與頂端專用的視窗拖曳把手。

### 新增
- **Windows 面板的更換面板按鈕現在會開啟完整控制選單**：點擊各主題內建的「更換面板」按鈕，會顯示本地化 HTML 覆蓋層，提供與 Windows 系統匣選單相同的面板、可見區塊、重新整理、通知、工作流程、更新與結束控制。
- **Windows 系統匣面板現在可移動**：無邊框面板頂端加入低調的拖曳把手，不會干擾額度卡片排序；位置會在下次開啟時還原並限制於目前工作區，也可從系統匣選單重設。
