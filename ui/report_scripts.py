...
REPORT_THEME_INIT_JS = """const reportThemeMedia = window.matchMedia('(prefers-color-scheme: light)');
let reportThemeChoice = null;
try {
  const saved = window.localStorage.getItem('usage-report-theme');
  if (saved === 'light' || saved === 'dark') reportThemeChoice = saved;
} catch (_) {
  // file:// or browser privacy settings can deny storage; use the system theme.
}
function applyReportTheme() {
  document.documentElement.dataset.theme = reportThemeChoice || (reportThemeMedia.matches ? 'light' : 'dark');
}
applyReportTheme();
"""

REPORT_JS_TEMPLATE = """const themeToggle = document.querySelector('[data-theme-toggle]');
function updateThemeToggle() {
  if (!themeToggle) return;
  const next = document.documentElement.dataset.theme === 'dark' ? 'light' : 'dark';
  themeToggle.querySelector('[data-theme-icon]').textContent = next === 'light' ? '☀' : '☾';
  themeToggle.querySelector('[data-theme-label]').textContent = themeToggle.dataset[next + 'Label'];
  themeToggle.setAttribute('aria-label', themeToggle.dataset[next + 'Aria']);
}
updateThemeToggle();
themeToggle?.addEventListener('click', () => {
  reportThemeChoice = document.documentElement.dataset.theme === 'dark' ? 'light' : 'dark';
  applyReportTheme();
  updateThemeToggle();
  try {
    window.localStorage.setItem('usage-report-theme', reportThemeChoice);
  } catch (_) {
    // Keep the selection for this page even when persistence is unavailable.
  }
});
reportThemeMedia.addEventListener('change', () => {
  if (reportThemeChoice !== null) return;
  applyReportTheme();
  updateThemeToggle();
});

const shareConfig = __SHARE_CONFIG_JSON__;
