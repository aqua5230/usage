...
### Fixed
- **Antigravity (Gemini) quota now shows 100% when the limit is hit**: rewrote quota estimation in two stages. (1) Replaced accumulated-context estimation with per-turn tracking — each `PLANNER_RESPONSE` is costed on only the new content since the previous response, eliminating the O(N²) inflation that caused 155%+ to appear in long conversations. (2) The underlying metric was still wrong because Antigravity's quota is "Work Done" (a proprietary compute unit), not tokens or dollars. The definitive fix: `gemini_loader` now parses `~/.gemini/antigravity-cli/log/cli-*.log` for `RESOURCE_EXHAUSTED` entries with `Resets in Xh Xm Xs`; when such a log event is found and the reset time hasn't passed, the quota bar is shown at exactly 100%. Between exhaustion events the percentage is estimated from API call count (empirical limits: ~360 calls / 5 h, ~3 600 calls / 7 d). The 5 h window is anchored to the last recorded reset timestamp so the estimate doesn't bleed across quota boundaries.
