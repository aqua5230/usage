...
- **ターミナル統合：** `usage status --json` は、コマンドを実行できるあらゆるツール——Starship、tmux、または独自のスクリプト——に Claude Code および Codex のクオータを渡します。メニューバーと同じローカルファイルを読み込み、ネットワーク呼び出しは行いません。[既製のスニペット](docs/DEVELOPMENT.md)。
